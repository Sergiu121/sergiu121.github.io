...
Fără aceste informații nu va ști ce utilitare să folosească pentru gesiunea aplicațiilor, nu va ști ce aplicații sunt disponibile, nu va ști unde sunt plasate fișierele de configurare.
...
În Linux, anumite aplicații au nevoie de o versiune dată a nucleului sistemului de operare (*kernel*).
...
sau folosi comanda ``uname`` cu opțiunea ``-a``:
...
Pot exista și alte arhitecturi de proces precum MIPS sau PowerPC.
Utilizatorul este, în general, nepreocupat de arhitectură, rareori având nevoie să știe acest lucru pentru o nevoie anume.
Dacă, însă, dorim să aflăm arhitectura procesorului sistemului, putem folosi comanda ``arch``:
...
Vedem în outputul comenzii că arhitectura procesorului/procesoarelor sistemului este ``x86_64``, arhitectura clasică pe sisteme desktop / laptop.
