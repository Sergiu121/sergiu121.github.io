...
Mai sus am afișat starea serviciului SSH. Observăm că acesta este activ și putem urmări ce procese a generat și mesaje de diagnosticare pe care le afișează.
...
Fișierul care gestionează serviciul SSH este în ``/lib/systemd/system/ssh.service`` și putem să îl identificăm din rezultatul comenzii ``systemctl status``.
...
.. code-block::
    student@uso:~$ sudo apt-get install ntp
    [...]
...
.. addmonition:: Observație
    ...
    Acesta este un serviciu important pentru buna funcționare a aplicațiilor în internet, deoarece atunci când o aplicație nu are timpul corect, comunicarea poate să nu funcționeze corect.
...
Pentru a verifica dacă mai funcționează serviciul SSH vom folosi comanda
``netstat`` pentru a verifica dacă mai ascultă serverul SSH pe portul ``22``.
...
Pornirea unei aplicații se face folosind comanda ``systemctl start`` în felul
următor:
...
Pe parcursul rulării serviciilor pe un sistem ne vom dori să schimbăm configurația unui serviciu sau să adăugăm parametri de rulare în plus.
Majoritatea aplicațiilor citesc fișierele de configurare o singură dată, atunci când sunt lansate, deci orice modificare ulterioară a fișierelor de configurare nu va fi sesizată de serviciu.
...
Acest lucru este dezactivat în mod predefinit de către serviciu din motive de securitate.
Pentru a face asta, avem nevoie să reconfigurăm serviciul SSH.
...
Odată ce am adăugat opțiunea ``PermitRootLogin yes`` în fișierul de configurare al serviciului și am dat restart serviciului, am reușit să ne autentificăm ca utilizatorul ``root``.
...
Acestea se pot găsi în mai multe locații:
...
* /etc/<numele serviciului>, unde se află fișierele de configurare pentru multe servicii instalate.
...
#) Configurați serviciul SSH astfel încât acesta să permită autentificarea ca utilizatorul ``root`` folosind numai autentificare bazată pe chei.
...
Noi vom găzdui propriul proxy care va trimite mesaje criptate prin SSH către o altă stație, de unde vor fi trimise mesaje în internet.
...
.. addmonition:: Atenție
    ...
    Pentru ca acest serviciu să funcționeze, este necesar să vă copiați cheia stației ``uso`` pe mașina ``fep.grid.pub.ro``.
